...
describe('GDIC CI Motor test', () => {
  ...
  // Select Vehicle Name as "Private Car"
  cy.log('Attempting to select Private Car in Vehicle Name');
  cy.get('ng-select[formcontrolname="mtrVehicleName"]').scrollIntoView().should('be.visible').click();
  
  // Wait for the dropdown options to load
  cy.get('.ng-dropdown-panel .ng-option', { timeout: 10000 }).should('be.visible');
  
  // Ensure we select the exact "Private Car" option
  cy.get('.ng-dropdown-panel .ng-option').each(($el, index, $list) => {
    const optionText = $el.text().trim();
    cy.log(`Vehicle Name option ${index}: ${optionText}`);
  
    if (optionText === 'Private Car') {
        cy.wrap($el).scrollIntoView().click({ force: true });
        cy.log('Private Car found and clicked!');
    }
  });
  
  // Verify the Private Care selection
  cy.get('ng-select[formcontrolname="mtrVehicleName"]').should('contain', 'Private Car', { timeout: 10000 });
  ...
  cy.get('.flex.mt-10 > :nth-child(2) > .select-group > .ng-select > .ng-select-container > .ng-value-container > .ng-input > input').type('{enter}')
    ...
    cy.screenshot('1st Quotation Private Car')
      
  
   //submission  of second page
   cy.get('.btn-primary').click()
  
    //Clauses I Mort Bank
    //Clauses I Mort Bank
    cy.get('.mr-10 > .ng-select > .ng-select-container > .ng-value-container > .ng-input > input', { timeout: 10000 })
    .should('exist')
    .should('be.visible')
    .type('2001{enter}');
     //add clause
     ...
     cy.get('.form-footer > :nth-child(2) > .btn').click()
     cy.screenshot('Preview_Private Car')
  ...
  cy.screenshot('Riot strike added preview_Private Car')
  cy.wait(1000)
  ...
  cy.screenshot('submitted_Private Car')
