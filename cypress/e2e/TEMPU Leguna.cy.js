...
describe('GDIC CI Motor test', () => {
    it('Logs in and navigates through the motor insurance module', function() {
        ...
        cy.get('.ng-dropdown-panel .ng-option', { timeout: 10000 }).contains('01538677835').click();
        cy.wait(2000)
        cy.get('ng-select[formcontrolname="businessType"]').scrollIntoView().should('be.visible').click();
        cy.get('.ng-dropdown-panel .ng-option').contains('New').click();
  ...
  // Select Vehicle Name as "Leguna"
  cy.get('ng-select[formcontrolname="mtrVehicleName"]', { timeout: 15000 }).should('be.visible').click();

cy.get('.ng-dropdown-panel .ng-option', { timeout: 15000 }).each(($el, index, $list) => {
  const optionText = $el.text().trim();
  cy.log(`Vehicle Name option ${index}: ${optionText}`);

  if (optionText.includes('Four wheeler')) {
    cy.wrap($el).scrollIntoView().click({ force: true });
    cy.log('Four wheeler(tempo/leguna) option found and clicked!');
  }
});

cy.get('ng-select[formcontrolname="mtrVehicleName"]').should('contain.text', 'Four wheeler', { timeout: 15000 });
  ...
  cy.get('input[formcontrolname="motorEngineAndChassisNumber"]')
    ...
    cy.get(':nth-child(2) > :nth-child(2) > :nth-child(4) > .input-group > .form-control').type(' HIACE TOYOTA')
  ...
  cy.get('ng-dropdown-panel')
    .find('.ng-option')
    .contains('TON')  // Find the option with text "TON"
    .click();        // Click on the option
  ...
  cy.get('ng-select[formcontrolname="mtrCcTon"]')
    .should('contain', 'TON');
  //TON value
    cy.get(':nth-child(12) > .input-group > .form-control').type('3')
  ...
  cy.get('.mt-5 > .btn-secondary').click()
    //get quotation
    cy.get('.mt-5 > .btn-secondary').click() 
    cy.screenshot('no calculation available for leguna')
    cy.screenshot('1st Quotation_LEGUNA')
   //submission 
  ...
  cy.screenshot('Riot strike added preview_LEGUNA')
  cy.wait(1000)
  ...
  cy.screenshot('submitted_LEGUNA')
